...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        """
        ctype, degrees_removed = 0, 0
        degree_a, degree_b = 1, 1
        aa, bb = self.names_to_nodes[a], self.names_to_nodes[b]
        if aa.get_parent() == None: acestor_a = aa
        else: acestor_a = aa.get_parent()
        if bb.get_parent() == None: acestor_b = bb 
        else: acestor_b = bb.get_parent()
        while True:
            if acestor_a == acestor_b.get_parent():
            if acestor_a == acestor_b: 
                ctype = min(degree_a, degree_b)
                degrees_removed = abs(degree_a - degree_b)
                break
            else:
                if acestor_a.get_parent() != None:  acestor_a, degree_a = acestor_a.get_parent(), degree_a + 1
                else: degree_a = degree_b + 7 
                if acestor_b.get_parent() != None:  acestor_b, degree_b = acestor_b.get_parent(), degree_b + 1
                else: degree_b = degree_a + 7
        return ctype-1, degrees_removed
